...
export const useAnimationHeroDiagram = () => {
  ...
  const animateSingleOutputDesktop = (outputLine: SvgNodeProps, index: number) => {
    ...
    timeline.set(outputLine, { labelVisible: false, onUpdate: () => updateOutputLine(index, { labelVisible: false }) }, 2.5)
    timeline.set(outputLine, { visible: false, onUpdate: () => updateOutputLine(index, { visible: false }) }, 3)
  ...
  const animateDiagram = () => {
    ...
    //@ts-ignore
    timeline.set(blueIndicator, { blueIndicator: true, onUpdate: () => setBlueIndicator(true) }, isMobile ? '>-2' : '>-0.2')
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    //@ts-ignore
    timeline.set(illuminateLogo, { illuminateLogo: true, onUpdate: () => setIlluminateLogo(true) }, '<-0.3')
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    //@ts-ignore
    timeline.set(pinkIndicator, { pinkIndicator: true, onUpdate: () => setPinkIndicator(true) }, '<+0.3')
    ...
    if (!isMobile) {
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      //@ts-ignore
      timeline.set(blueIndicator, { blueIndicator: false, onUpdate: () => setBlueIndicator(false) }, '>-1')
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      //@ts-ignore
      timeline.set(pinkIndicator, { pinkIndicator: true, onUpdate: () => setPinkIndicator(true) }, '<')
      timeline.set({}, {}, '+=0.2')
    }
  }
  useEffect(() => {
